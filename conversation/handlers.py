...
def stickers(update, context):
    ...
    if DB.firstTimeEntry(chat_id):
        reply_markup = ReplyKeyboardMarkup(
            keyboard=stickers_kb, resize_keyboard=True, one_time_keyboard=True
        )
...
def current_mood(update, context):
    ...
    reply_markup = ReplyKeyboardMarkup(
        keyboard=current_mood_kb, one_time_keyboard=True, resize_keyboard=True
    )
    update.message.reply_text(text=current_mood_text, reply_markup=reply_markup)
...
def to_google_sheet(update, context):
    ...
    answers = [
        google_answers["why_leave"][answers[-2]],
    ...
    reply_markup = ReplyKeyboardMarkup(
        keyboard=funny_kb, one_time_keyboard=True, resize_keyboard=True
    )
