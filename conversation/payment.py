...
def noSubscription(update, context):
    ...
    month = InlineKeyboardButton(
        text=f"Месяц - {4*currency} RUB", callback_data="month"
    )
...
def no_sps(update, context):
    ...
    b2 = InlineKeyboardButton(
        text="Меня не устраивает цена подписки", callback_data="reason2"
    )
...
def other(update, context):
    ...
    reason_text = {
        "reason1": "Сервис мне не интересен",
        "reason2": "Меня не устраивает цена подписки",
    }
...
def pay(update, context):
    ...
    description = {
        "3days": [
            "Будет действовать в течение 3х дней 🦀",
            LabeledPrice("Subscribe on 3 days", 100 * currency),
        ],
        "week": [
            "Будет действовать в течение недели 🐟",
            LabeledPrice("Subscribe on week", 200 * currency),
        ],
        "month": [
            "Будет действовать в течение месяца 🐬",
            LabeledPrice("Subscribe on month", 400 * currency),
        ],
    }
...
def successful_payment_callback(update, context):
    ...
    update.message.reply_text(
        "Как хорошо, что ты здесь!) Узнаем твоё настроение?🦄", reply_markup=reply_markup
    )
