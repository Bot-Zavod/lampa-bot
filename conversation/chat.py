...
def start(update, context):
    update.message.reply_text(
        connection_text, reply_markup=ReplyKeyboardRemove())
...
def answer(update, context):
    if update.message.chat_id in users:
        context.bot.send_message(chat_id=users[update.message.chat_id], text=update.message.text)
    else:
        context.bot.send_message(chat_id=update.message.chat_id, text="Sorry, we didin`t find a companion for you yet")
    return None

def stick_answer(update, context):
    sticker_id = update.message.sticker.file_id
    if update.message.chat_id in users:
        context.bot.send_sticker(chat_id=users[update.message.chat_id], sticker=sticker_id)
    else:
        context.bot.send_message(chat_id=update.message.chat_id, text="Sorry, we didin`t find a companion for you yet")
    return None
...
def done(update, context):
    ...
    if chat_id in lobby:
        ...
        update.message.reply_text(chat_finished_text)
    update.message.reply_text(why_leave_text, reply_markup=ReplyKeyboardMarkup([list(map(str,range(1,6)))], resize_keyboard=True))
    print(users, lobby)
...
chat_handler = ConversationHandler(
    ...
    states={
        CONVERSATION: [MessageHandler(Filters.text & (~Filters.command),answer),MessageHandler(Filters.sticker,stick_answer)],
    },
    fallbacks=[CommandHandler('stop', done)],
    map_to_parent={
            ConversationHandler.END: States.WHY_LEAVE,
        }
)